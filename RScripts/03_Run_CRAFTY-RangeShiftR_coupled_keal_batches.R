...
if (Sys.info()["user"] %in% c("alan", "seo-b")) { 
  ...
  dirOutput <- "/DATA4TB/CRAFTY_RangeshiftR_21-22_outputs"
   # 
  # dirWorking <- "/pd/data/crafty/CRAFTY_RangeshiftR"
  # # dirOutput <- "/pd/data/crafty/CRAFTY_RangeshiftR_21-22_outputs"
...
foreach(batch_idx = batches_todo, .errorhandling = "stop", .verbose = T) %do% {

  # dirCRAFTYOutput <- path.expand(dataDisk)
  # dirCRAFTYOutput <- path.expand(paste0(dirWorking, "/output"))
   
  dirCRAFTYOutput = paste0(dirOutput, "/Batch_",batch_idx)
  
  if (!dir.exists(dirCRAFTYOutput)) { 
    dir.create(dirCRAFTYOutput, recursive = T)
  }
  ...
  foreach(pref =prefs_todo, .errorhandling = "stop", .verbose = T) %do% {
    ...
    foreach(s.idx = scenarios_todo, .errorhandling = "stop",
            ...
            , .verbose = T) %dopar% {
              ...
              # copy production functions
              prod_path = paste0(dirCRAFTYInput, "/batches/production_batches/", batch_idx)
              file.copy(from=file.path(list.files(prod_path, pattern = ".csv$", full.names = T)), to = file.path(paste0(dirCRAFTYInput, "/production/", scenario.split, "/")), overwrite = T)
              ...
              if (file.exists(dirCRAFTYscenario)){
                ...
                dir.create(file.path(dirCRAFTYscenario), recursive = T)
              ...
              for (CRAFTY_tick in 1:length(timesteps)) {
                ...
                # I believe the below was a potential source of the error. (26 Jan 2022 by ABS).
                # capitals <- read.csv(paste0(dirCRAFTYInput,"worlds/GreaterLondon/",scenario.split,"/GreaterLondon_tstep_",CRAFTY_tick,".csv"))
                
                # Instead, read the immutable initial capitals. Otherwise previous simulations could affect future ones.
