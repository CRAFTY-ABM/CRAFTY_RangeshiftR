...
dataDrive <- "/DATA4TB/CRAFTY_Rangeshifter_output_v10_r25_0.1_30years/"
dirFig_root = "~/Dropbox/"
...
lstScenarios <- c("baseline-with-social","baseline-no-social",
                  ...
                  "un-coupled-with-social","un-coupled-no-social")[c(1,3,5,7)]
...
foreach (pref_idx = prefs_todo, .packages = c("dplyr", "ggplot2", "tidyverse", "viridis", "sf")) %dopar% {
  ...
  for (idx in 1:length(lstScenarios)){
    ...
    foreach (y_idx = 1:n_years) %dopar% { 
      year <- lstRsftrYrs[y_idx]
      
       dfResults_y_tmp = merge(dfResults[dfResults$Tick == y_idx,], dfCoords, by.x = c("X", "Y"), by.y = c("X", "Y"))
      ...
      gAFT = ggplot(sfAFT_tmp[,]) + 
        ...
        geom_sf(aes(fill=factor(AFT))) + #, colour=NA)+
        scale_fill_manual(values =agent.pal, name= "AFT")+
