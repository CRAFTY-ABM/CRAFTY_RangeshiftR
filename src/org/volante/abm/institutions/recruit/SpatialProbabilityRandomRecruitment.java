...
public class SpatialProbabilityRandomRecruitment implements
	...
	public Collection<InnovativeBC> getRecruitedAgents(
		...
		for (InnovativeBC ibc : allAgents) {
			if (ibc.getAgent().getHomeCell().getX() >= this.xMin
					&& ibc.getAgent().getHomeCell().getX() <= this.xMax

					&& ibc.getAgent().getHomeCell().getY() >= this.yMin
					&& ibc.getAgent().getHomeCell().getY() <= this.yMax
